  ...
  const app = (function () {
    ...
    return {
      ...
      setDescription: function setDescription() {
        ...
        $gamesDom.innerHTML = selectedGame.description
      ...
      addToCart: function addToCart() {
        if (!selectedGame.type) {
          alert(
            "Você deve selecionar um jogo e escolher os números em que vai apostar"
          );
          return;
        }
        if (selectedGame["max-number"] > gameNumbers.length) {
          alert(
            `Falta escolher mais ${
              selectedGame["max-number"] - gameNumbers.length
            } números`
          );
          return;
        }
        ...
        $cartGames.innerHTML += `
            ...
            <img src="../assets/icons/trash.svg" alt="Trash Icon">
              ...
              <p>${orderedNumbers.join(", ")}</p>
                <p style="color: ${selectedGame.color}">${
          selectedGame.type
        } <span>${app.formatPrice(selectedGame.price)}</span></p>
              </div>
            </li>
          `;
      ...
      showTotalValueBet: function showTotalValueBet(value) {
        ...
        if (totalValueBet !== 0) {
          $total.innerHTML = `
            <p><strong>CART</strong> Total: ${app.formatPrice(
              totalValueBet
            )}</p>
            `;
          return;
      ...
      removeGame: function removeGame(event) {
        ...
        totalValueBet -= parseFloat(
          $removedLi.lastElementChild.lastElementChild.lastElementChild.textContent
            .slice(3)
            .replace(",", ".")
        );
        $removeButton.parentNode.parentNode.removeChild(
      ...
      addNumber: function addNumber(event) {
        ...
        if (hasInArray) {
          ...
          const numberIndex = gameNumbers.findIndex(
      ...
      completeGame: function completeGame() {
        ...
        while (
          ...
          const selectedNumber = app.getRandomIntInclusive(
            0,
            selectedGame.range
          );
          const hasInArray = gameNumbers.some((item) => {
      ...
      saveGame: function saveGame() {
        if (totalValueBet === 0) {
          alert(
            "Seu carrinho está vazio. Aposte um pouco, só não ganha quem não joga!!"
          );
          return;
        }
        if (totalValueBet < games.types[0]["min-cart-value"]) {
          alert(
            `O valor mínimo de aposta é ${app.formatPrice(
              games.types[0]["min-cart-value"]
            )}`
          );
          return;
        }
